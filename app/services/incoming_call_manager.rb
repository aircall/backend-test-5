...
# IncomingCallManager service which contains
...
class IncomingCallManager
  ...
  def give_choice
    @resp.gather(num_digits: 1, timeout: 4, action: @process_selected_choice_path) do |gather|
      gather.say(message: 'If you want to be forwarded, press 1. If you prefer to leave a message, press 2.',
                 voice: 'alice')
    end
  ...
  def voice_mail_redirection
    @resp.say(message: 'Please leave a message at the beep. Press the number sign key when finished.')
